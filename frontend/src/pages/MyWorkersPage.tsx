...
import AssignedWorkerCard from '../components/molecules/AssignedWorkerCard';

const MyWorkersPage: React.FC = () => {
  const { user } = useAuth();
  ...
  const loadAssignedWorkers = async () => {
    try {
      const data = await getAssignedWorkers();
      setAssignedWorkers(data);
    } catch (err) {
      console.error('Error al cargar trabajadores asignados:', err);
      setAssignedWorkers([]); // Establecer array vacío en caso de error
    }
  };
  ...
  const handleWorkerContact = async (worker: AssignedWorker) => {
    if (!user) return;
    
    setContactLoading(worker._id);
    
    try {
      // Buscar si ya existe una conversación entre el usuario y el trabajador
      const conversaciones = await conversacionService.obtenerConversacionesUsuario(user._id);
      
      // Buscar conversación existente con este trabajador
      const conversacionExistente = conversaciones.find(conv => 
        conv.participantes.some(p => p._id === worker._id)
      );
      ...
      if (conversacionExistente) {
        ...
        notifications.show({
          title: 'Conversación encontrada',
          message: `Te has unido a la conversación con ${worker.fullName}`,
          color: 'green',
        });
      } else {
        // Si no existe, crear una nueva conversación
        const nuevaConversacion = await conversacionService.crearConversacion({
          participantes: [user._id, worker._id],
          metadata: {
            tipo: 'consulta',
        ...
        notifications.show({
          title: 'Conversación creada',
          message: `Se ha creado una nueva conversación con ${worker.fullName}`,
          color: 'green',
        });
      }
    } catch (error) {
  ...
  const handleWorkerAppointment = (worker: AssignedWorker) => {
    // Redirigir a la página de citas con el trabajador preseleccionado
  ...
  // Si es trabajador, mostrar solo las solicitudes
  if (user?.role === 'worker') {
    return (
      <Container size="lg" py="xl">
        <Stack gap="xl">
                ...
                <Card key={request._id} p="lg" radius="lg" withBorder>
                  <Stack gap="md">
                    <Group justify="space-between" align="flex-start">
                      <Group gap="md">
                        <Avatar
                          src={request.usuarioSolicitante.profilePicture}
                          size="lg"
                          radius="xl"
                        >
                          {request.usuarioSolicitante.fullName.charAt(0).toUpperCase()}
                        </Avatar>
                        <Box>
                          <Title order={4} mb="xs">
                            {request.usuarioSolicitante.fullName}
  ...
  // Si es usuario, mostrar pestañas con trabajadores asignados y solicitudes
  return (
    <Container size="lg" py="xl">
      <Stack gap="xl">
        <Box>
            ...
            {assignedWorkers.length === 0 ? (
              <Card p="xl" radius="lg" withBorder>
                <Stack align="center" gap="md">
                  ...
                  <Text c="dimmed" ta="center">
                    Aún no tienes trabajadores asignados. Envía solicitudes para comenzar.
                  </Text>
                </Stack>
              </Card>
            ) : (
              <Stack gap="md">
                {assignedWorkers.map((worker) => (
                  <AssignedWorkerCard
                    ...
                    onAppointment={handleWorkerAppointment}
                    contactLoading={contactLoading === worker._id}
                  />
                ))}
              </Stack>
            )}
                  ...
                  <Card key={request._id} p="lg" radius="lg" withBorder>
                    <Stack gap="md">
                      <Group justify="space-between" align="flex-start">
                        <Group gap="md">
                          <Avatar
                            src={request.trabajadorSolicitado.profilePicture}
                            size="lg"
                            radius="xl"
                          >
                            {request.trabajadorSolicitado.fullName.charAt(0).toUpperCase()}
                          </Avatar>
                          <Box>
                            <Title order={4} mb="xs">
                              {request.trabajadorSolicitado.fullName}
